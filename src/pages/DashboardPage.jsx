...
export default function DashboardPage() {
  ...
  const handleExport = async (type) => {
    ...
    try {
      ...
      if (type === 'csv') { await exportMoodCSV(user.uid, name); toast.success('CSV downloaded! 📊') }
      else { await exportMoodPDF(user.uid, name, profile?.streak || 0, profile?.totalLogs || 0); toast.success('PDF report opened! 📄') }
        ...
        <div className="card animate-fade-up delay-400">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
              ...
              <p className="text-xs text-slate-500 mt-0.5">Download your mood history as CSV or a beautiful PDF report</p>
            </div>
            <div className="flex gap-2">
              ...
              <button onClick={() => handleExport('pdf')} disabled={!!exportLoading}
                ...
                {exportLoading === 'pdf' ? '⏳ Generating...' : '📄 PDF Report'}
              </button>
            </div>
          </div>
        </div>
